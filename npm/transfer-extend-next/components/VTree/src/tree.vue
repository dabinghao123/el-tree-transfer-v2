<template>
  <div class="el-tree" :class="{
    ...
    <RecycleScroller v-if="!isEmpty" :style="{
    height,
    'overflow-y': 'auto',
    'scroll-behavior': 'smooth',
  }" class="scroller-container" :items="treeData" :item-size="itemSize" key-field="key" :buffer="buffer">
      <template slot-scope="{ item , active}">
        <ElTreeNode class="test2" v-if="active" :node="item" :props="props" :render-after-expand="renderAfterExpand"
          :show-checkbox="showCheckbox" :key="getNodeKey(item)" :render-content="renderContent"
...
import { getNodeKey, findNearestComponent } from "./model/util.js";
...
export default {
  ...
  computed: {
    ...
    treeData() {
      let list = this.flatAllTree(this.root.childNodes);
      return list;
    },
  },
  ...
  methods: {
    ...
    flatAllTree(treeData) {
      return treeData.reduce((smoothArr, data) => {
        ...
        if (data.expanded && data.childNodes.length) {
          smoothArr.push(...this.flatAllTree(data.childNodes));
        }
    ...
    handleNodeExpand(nodeData, node, instance) {
      ...
      if (this.accordion) {
        this.treeData.forEach(Node => {
          if (Node.parent === node.parent && node !== Node) {
            Node.expanded = false
          }
        })
      } else {
    ...
    // 获取可见节点数据
    getVsibleNodesData(isleaf) {
      const nodes = this.store.root;
      const allData = [];
      function getNodeData(nodes) {
        for (let i = 0; i < nodes.length; i++) {
          const { visible, isLeaf, data, childNodes } = nodes[i];
          if (!visible) continue;
          if (isleaf === true) {
            if (isLeaf === isleaf) allData.push({ ...data });
          } else {
            allData.push({ ...data });
          }
          if (Array.isArray(childNodes) && childNodes.length) {
            getNodeData(childNodes);
          }
        }
      }

      if (!Array.isArray(nodes) && nodes.childNodes) {
        getNodeData(nodes.childNodes);
      } else {
        getNodeData(nodes);
      }
      return allData;
  ...
  created() {
    ...
    this.store = new TreeStore({
      ...
      checkNotMatchNode: this.checkNotMatchNode,
    });
